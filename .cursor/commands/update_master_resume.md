...
4. **Merge intelligently**:
   ...
   - Prefer `impactEntries` over `highlights` for experience and projects when the data is rich enough to decompose into challenge / action / result / metric / scope.
   - Preserve existing `highlights` unless the user explicitly asks to replace or remove them.
5. **Enrich where possible** — if the input contains enough explicit context to populate optional fields (`metric`, `scope`, `skills` inside an impactEntry, `tags`, `evidence`), fill them in.
6. **Validate before write** — validate the merged JSON against `src/resume_schema.json`. If invalid, either fix deterministically from known data or ask a targeted clarifying question before writing.
7. **Write the result** — overwrite `src/resumes/master_resume.json` with the merged JSON, pretty-printed with 2-space indentation.
8. **Summarise changes** — after writing, tell the user what was added, updated, or left unchanged in plain language.

## Rules

- Output must be valid JSON that conforms to `src/resume_schema.json`.
- Do not invent or hallucinate details not present in the input or the existing file.
